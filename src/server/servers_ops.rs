...
pub(crate) struct GetServerMetricsArgs {
    ...
    pub id: u64,
    /// Metric types to fetch: cpu, disk, network. Comma-separate for more
    /// than one, e.g. "cpu,disk".
    #[serde(rename = "type")]
    pub r#type: String,
...
impl HcloudServer {
    ...
    #[tool(
        ...
        time period. type accepts a comma-separated list (e.g. \"cpu,disk\"); \
        start/end are RFC3339 timestamps. Metrics are available for the last \
        30 days only.",
        annotations(
    ...
    pub(crate) async fn get_server_metrics(
        ...
        for t in args.r#type.split(',') {
            if !METRIC_TYPES.contains(&t) {
                return Err(ErrorData::invalid_params(
                    format!(
                        "type must be a comma-separated list of {}, got {:?}",
                        METRIC_TYPES.join(", "),
                        args.r#type
                    ),
                    None,
